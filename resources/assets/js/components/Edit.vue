    ...
    <div class="row">
      <div class="row mt-3 mb-3">
        <div class="form-group">
          ...
          <input class="form-control" v-model="medium.name">
          <small class="text-danger" v-if="errors.name">{{ errors.name[0] }}</small>
        </div>
      </div>

      <div class="row mt-3 mb-3 px-5">
        <button type="button" class="btn btn-primary" @click="editMedium" :data-kt-indicator="is_loading? 'on':'off'" :disabled="is_loading">
...
export default {
  name: "Edit",
  props: {
    mediumData: {required: true}
  },
  created() {
    this.medium = Object.assign({}, this.mediumData)
  },
  data: () => ({
    is_loading: false,
    medium: {
  ...
  methods: {
    async editMedium() {
      ...
      await axios.patch(route('dashboard.media.update', this.mediumData.id), this.medium).then((res) => {
        window.location.href = route('dashboard.media.index')
      }).catch((err) => {
